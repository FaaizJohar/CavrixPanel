#!/usr/bin/env bash
# =============================================================================
#  Pigeon Panel - All-in-one installer for VPS + Custom Domain + Cloudflare Tunnel
#  Tested on Ubuntu 22.04 / 24.04 and Debian 12.
#
#  The VPS is NOT exposed to the internet. Cloudflare Tunnel (cloudflared)
#  makes an outbound connection to Cloudflare's edge, so no ports need to be
#  opened and TLS is terminated at the edge. The custom domain must be on a
#  Cloudflare zone (nameservers pointed at Cloudflare).
#
#  Handles: system deps, PHP 8.3, Nginx (loopback), MariaDB, Redis, Composer,
#  panel install, database, Cloudflare tunnel + DNS routing, cron, queue
#  worker, admin user.
#
...
#                         (default: ./pigeon-panel-deploy.zip next to script)
#    --tunnel-name <name> Default: pigeon-panel
...
[[ $EUID -eq 0 ]] || die "Please run as root: sudo bash install-cloudflare.sh $*"
...
# ---- prompts ----------------------------------------------------------------
[[ -n "$DOMAIN" ]] || read -r -p "Enter the panel domain (must be on Cloudflare, e.g. panel.example.com): " DOMAIN
[[ -n "$DOMAIN" ]] || die "A domain is required."
[[ -n "$EMAIL" ]] || read -r -p "Enter your email (used for the admin user): " EMAIL
...
apt-get update -qq
apt-get install -y -qq curl wget git unzip zip cron nginx mariadb-server redis-server software-properties-common ca-certificates lsb-release gnupg2 >/dev/null
...
# ---- .env (https via tunnel, trust loopback proxy) ---------------------------
...
# ---- nginx (loopback only - reached only via the tunnel) ---------------------
info "Configuring Nginx on 127.0.0.1:${ORIGIN_PORT}..."
...
server {
    ...
    location ~ \.php$ {
        ...
        # Cloudflare terminates TLS at the edge; the tunnel always delivers https.
...
ok "Nginx configured (loopback only)"
...
echo ""
echo "============================================================"
echo "  Pigeon Panel installed via Cloudflare Tunnel!"
echo "============================================================"
...
echo "============================================================"
echo "  Notes:"
echo "  * Your domain must already be on Cloudflare (NS pointed to Cloudflare)."
echo "  * DNS may take a few minutes to propagate before https://$DOMAIN works."
echo "  * Check the tunnel:  cloudflared tunnel info $TUNNEL_NAME"
echo "  * Tunnel logs:       journalctl -u cloudflared-panel -f"
echo "  * No ports are open on this VPS; nothing is exposed except through the tunnel."
echo "============================================================"
